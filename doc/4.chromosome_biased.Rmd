...
save(chr_results, file = "M:/GOcategoryStats/data/chr_results.rda")
```
...
```{r check_results}
ifelse(using_laptop, 
       load("C:/Users/bigginsl/Desktop/temp/GOcategoryStats/data/chr_results.rda"),
       load("M:/GOcategoryStats/data/chr_results.rda")
)
...
```{r p_values, fig.height = 30, results = FALSE, eval = FALSE}
...
Remove the categories that appear fewer than 10 times
```{r}
keeper_categories <- lapply(tabled_categories, function(x) names(x[x >= 10]))
```
...
```{r}
filtered_categories <- lapply(tabled_categories, function(x) x[x >= 10])
...
```{r}
suspects <- lapply(tabled_categories, function(x) names(x[x >= 10]))  
...
for (i in 1:length(suspects)) {
   ...
   ifelse(using_laptop,
          filename <- paste0("M:/biased_gene_lists/identified_categories/chr", names(suspects)[i], ".txt"),
          filename <- paste0("C:/Users/bigginsl/Desktop/temp/biased_gene_lists/identified_categories/chr", names(suspects)[i], ".txt")
   )
