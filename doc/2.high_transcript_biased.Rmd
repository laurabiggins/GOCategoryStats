...
In the mouse genome, most genes have only one transcript, but 
Do genes with high numbers of transcripts 
more power, more reads 
...
#genfo <- read.delim("../data/Mus_musculus.GRCm38.94_gene_info.txt")
...
The first set of thresholds that I generated
```{r}
thresholds <- c("10" = 10, "15" = 15, "20" = 20)
sapply(thresholds, function(x) sum(genfo$no_of_transcripts >= x))
```
...
200 genes.
When I did test a min20 category, almost all the categories came up in the min15
set too.


####Generating thresholds using the distribution of the data rather than just picking arbitrary thresholds.
This method is more robust for applying to different species and future versions 
of this genome.
```{r}
...
```{r, eval = FALSE}
ggplot(genfo, aes(x = no_of_transcripts)) + geom_histogram(fill = "seagreen", binwidth = 1, color = "darkblue") + scale_y_log10()

ggplot(genfo, aes(x = no_of_transcripts)) + geom_freqpoly(binwidth = 3) + scale_y_log10()
```


```{r}
dens <- density(genfo$no_of_transcripts, adjust = 5)
plot(dens, xlim = c(-2,30))
```
...
min10_genes <- sapply(1:100, function(i){
  all_min10[ceiling(runif(200, min = 0, max = length(all_min10) - 1))]
})

min15_genes <- sapply(1:100, function(i){
  all_min15[ceiling(runif(200, min = 0, max = length(all_min15) - 1))]
})

biased_transcripts <- list(min10 = min10_genes, min15 = min15_genes)
...
```{r, fig.height=9}
head(genfo[match(min10_genes[,5], genfo$gene_name),])
tail(genfo[match(min10_genes[,12], genfo$gene_name),])
head(genfo[match(min15_genes[,28], genfo$gene_name),])
tail(genfo[match(min15_genes[,73], genfo$gene_name),])
```
...
```{r, eval=FALSE}
save(biased_transcripts, file = "M:/temp/biased_transcripts/biased_transcripts.rda")

x <- overrep_test(all_go_categories, query_genes = biased_transcripts$min15[,1], bg_genes)
...
names(transcript_results) <- c("min10", "min15")
...
There are `r print_overlaps(suspects$min15, suspects$min10)` overlapping categories
between min15 and min10 so I'll remove them from the min15 category.
```{r}
suspects$min15 <- with(suspects, min15[!min15 %in% min10])
...
for (i in 1:length(suspects)){
  ...
  ifelse(using_laptop,
         filename <- paste0("C:/Users/bigginsl/Desktop/temp/biased_gene_lists/identified_categories/", names(suspects)[i], "transcripts.txt"),
